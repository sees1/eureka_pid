  ...
  mppi::ParametersHandler * param_handler)
{
  ...
  max_robot_pose_search_dist_ = getMaxCostmapDist(); // hardcode
...
PathHandler::getGlobalPlanConsideringBoundsInCostmapFrame(const geometry_msgs::msg::PoseStamped & global_pose)
{
  ...
  for (auto global_plan_pose = closest_point; global_plan_pose != pruned_plan_end; global_plan_pose++)
  {
    ...
    if (!costmap_->getCostmap()->worldToMap(costmap_plan_pose.pose.position.x,
                                            costmap_plan_pose.pose.position.y,
                                            mx,
                                            my))
    {
...
{
  // global_plan_up_to_inversion - это обрезка пути до ближайшей точки инверсии
  // inversion_local - сдвиг от global_plan_up_to_inversion.begin() до ближайшей точки инверсии (см по поиску)
  // функция корректная - ничего не трогать!!!
